...
# Introduction
America has a strong tradition of activism, dating back to[slave revolts](https://www.pbs.org/wnet/african-americans-many-rivers-to-cross/history/what-was-americas-1st-black-town/) and
...
as well as day to day [digital](https://www.wikihow.com/Be-Safe-on-the-Internet) [safety](https://www.wikihow.com/Protect-Personal-Information).  
...
## Tools NOT to Use
In this document, we make recommendations against some wildly popular tools. Our recommendations
...
Another popular tool we must recommend against is Telegram. [Telegram has multiple issues with security](https://www.wired.com/story/telegram-encryption-whatsapp-settings/).
While it supports end-to-end encryption (see [Messaging with Encryption](#messaging-with-encryption), below)
for messages between individuals, it does not apply this security to group messages. Group
messages can be flagged as “spam” or “abuse” by Telegram staff & contractors, which means that
their staff and contractors can see the contents of the messages. That data can be provided to
law enforcement. To make things worse, the full encryption technique is not active for any
conversation by default, so it is not secure out of the box and [we cannot recommend it](https://gizmodo.com/why-you-should-stop-using-telegram-right-now-1782557415).
...
we probably researched its security features for this project. If there are tools you are not
...
you have been, what you have said, and whom you have communicated with. This section will help
...
Assume that any messages you send, phone calls that you make, or anything else you send or
receive can be logged & recorded by the police. If your information is strongly encrypted, they
...
## Do’s and Don’ts
* **Do not “check in” to a protest on Facebook** or any other service. You don’t want to give
  third parties evidence that you were there. Police use subpoenas to get user information from
  Facebook and other social media companies all the time.
* **Don’t post photos or videos.** These have extra information hidden in them that includes
  the time and place they were taken. If you send photos or videos out at all, only send them to
  people you trust, over secure means, long after and far away from the protest. 
* **Don’t take anyone’s photo without permission.** 
* **Do have a protest buddy and use Signal to communicate with them.**
* **Do write an emergency contact’s number somewhere conspicuous** (piece of paper, on your arm, etc)
  in case your device is confiscated or lost.  If you are at a high-risk event, also bring the
  phone number of your local chapter of the [National Lawyers Guild](https://www.nlg.org/)
  (in case of arrest).
...
* Set up a [Signal](https://signal.org/en/) account. If you don’t want to use your personal
  phone number, you can create a new phone number with Google Voice and register that with Signal.
...
* Use [Apple Maps](https://maps.apple.com/go) or [DuckDuckGo Maps](https://help.duckduckgo.com/duckduckgo-help-pages/features/maps/) if you need directions to the location of the protest. 
	* If you can, print the directions. While Apple Maps does not have an explicit Offline mode,
	if you route to a location it will retain the directions and surrounding map data.
...
share advice to make your phone as strong as possible against intrusion should it fall into
...
has a **GPS** reciever built into it that enables maps and other location-based apps to work.
The phone itself will keep that GPS recieveer on at all times, recording where you are, and
occasionally send updates on your whearabouts to Google, Apple, or other app publishers. So we
will want to turn the GPS reciever off, which prevents from the location data from being
recorded at all. 
...
Companies have deployed Bluetooth beacons in their stores to identify how long customers linger
in certain departments, or in front of certain displays, and then correlate that data with the
person's other identifying information. This type of surveillance is profoundly dangerous at an
...
This information can be accessible to law enforcement through a subpoena. This data is slightly 
more difficult for law enforcement to get than Bluetooth or Wifi, but it can be done.
...
operation. This feature is useful today for being a reliable way to disable all three radios
with one button: WiFi, Bluetooth, and mobile carrier. There are some slight caveats, detailed
below, of which the biggest is that airplane mode usually does not disable the GPS receiver.
Generally, airplane mode is an important tool to prevent generation and collection of tracking
data.

## Intrusion Protection
If you attend an action, there is a risk that your phone may be seized by police. They
physically have your phone and may attempt to extract information from it. All phones have
**lock screens**, which is the first line of defense. In day-to-day use, we want our lockscreen
to be secure against a thief, or a taxi driver, or some other stranger, from easily opening the
phone. With that, fingerprint readers and face scanners make for convenient unlock mechanisms.
The situation at an action is starkly different, as law enforcement may have detained you with
your phone, and therefore may have the ability to apply your finger or face to your phone to
...
are typically drawing a shape or entering a code. **Entering a code is the most secure**, as the
shape can be traced from fingerprints on the screen. The code should also be long enough to
prevent your fingerprints from giving it away, too. Related, you want your screen to lock
quickly on its own. The **power button should make the phone immediately lock**, so that you can
lock it quickly. Phones also let you select how long they should wait before auto-locking
themselves. This time should be as short as possible, so that if you drop your phone or if it is
stolen, the timer will expire and the phone will lock before anyone else can access it.  

There are ways to access data on a phone without unlocking it. One way to defend against these
methods is **full-disk encryption**, where all the information on the phone is stored encrypted.
When the phone is turned on, nothing can be read until the screen is unlocked the first time, at
which point it will decrypt information as needed, and continue to store information in
encrypted form. This prevents law enforcement from removing the storage chips from your phone,
as all of the contents will be unreadable without being unlocked in your specific phone.
...
To see if your Android has encryption available and turned on, you can **Go to Settings, choose
Security, and find the Encrypt Phone option.** It will say “Encrypted” or prompt you to turn on
...
iPhone-specific features to make securing your phone easier. These steps below can be used to
...
and social meet-ups but do not offer the privacy activism requires. For that reason, we
...
participants need to have accounts or give any personal information to Jitsi to use the
platform. They take security and personal anonymity seriously.
...
app on your phone, text message, or email. A one-time-use code is sent to you which you then
...
When choosing a 2FA method, an authenticator app such as Google Authenticator ([Android](https://play.google.com/store/apps/details?id=com.google.android.apps.authenticator2),
[iOS](https://apps.apple.com/us/app/google-authenticator/id388497605)) provides a good balance
between convenience and security. If more security is desired, physical tokens such as the
[Yubikey](https://www.yubico.com/products/) can be used, and the [Yubico Authenticator](https://www.yubico.com/products/services-software/download/yubico-authenticator/)
can use the physical tokens to generate one-time codes.

If neither of the above options is available for your service, you may be able to use your email
...
* Don’t use the built-in cloud backup solutions for Android or iOS. The most secure data backup
  is the one you make over a USB cable.
* Google your full name occasionally and see what comes up. 

# Closing 
We hope that you have found this resource helpful. While at first these materials could seem
...
and the ACLU tackles [ongoing threats to protestor's rights](https://www.aclu.org/podcast/deray-mckesson-threat-protesters-rights-ep-99).
For ongoing news in the world of privacy and freedom of speech, check out the below resources!  
