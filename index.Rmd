...
Your phone is a powerful tool, but also a powerful tracking device for law enforcement and other entities. Phones typically record and share lots of information about your whearabouts and activity with different services. This is usually a good thing, as it allows your apps to be responsive to your needs and situation. In regards to an action, those features can provide a rich digital breadcrumb trail that is accessible to law enforcement. We want to prevent those digital breadcrumbs from being dropped, and here is some advice on how to do that. 
...
This section uses the word "phone" to mean a smartphone running iOS or Android, such as those manufactured by Apple, Google, Samsung, and others. If you are using a simpler phone, you may only be subject to a subset of the threats discussed below. 
...
**Bluetooth and WiFi** are common technologies, using short-range radio to communicate with other devices. But their operation requires your phone to broadcast identifiable information about itself. This information can be observed by a "beacon" or scanning device, which are used by stores, government agencies, and police to identify people who are nearby. Companies have deployed bluetooth beacons in their stores to identify how long customers linger in certain departments, or in front of certain displays, and then correlate that data with the person's other identifying information. This is profoundly dangerous at an action, so we need to disable Bluetooth and WiFi whenever possible. 
...
All phones have an option for **"airplane mode,"** which disables *all* radios. This feature was created for travel in an airplane, out of a concern that phones would interfere with the plane's operation. This feature is useful today for being a reliable way to disable all three radios with one button: WiFi, Bluetooth, and mobile carrier. There are some slight caveats, detailed below, of which the biggest is that airplane mode usually does not disable the GPS reciever. Generally, airplane mode is an important tool to prevent tracking data from being generated or collected.

## Intrusion Protection

If you attend an action, there is a risk that your phone may be siezed by police. This situation is different from above, as they physically have your phone and may attempt to extract information from it. All phones have lock screens, which is the first line of defense. In day-to-day use, we want our lockscreen to be secure against a theif, or a taxi driver, or some other stranger, from easily opening the phone. With that, fingerprint readers and face scanners make for convenient unlock mechanisms. The situation at an action is starkly different, as law enforcement may have detained you with your phone, and therefore have the ability to apply your finger or face to your phone to unlock it, even if you resist or are unconscious. These sort of biometric unlock mechanisms are not secure in this situation, and should be avoided during an action. The other unlock mechanisms are typically drawing a shape or entering a code. The code is the most secure, as the shape can be traced from fingerprints on the screen. The code should also be long enough to prevent your fingerprints from giving it away, too. Related, you want your screen to lock quickly on its own. The power button should make the phone immediately lock, so that you can lock it quickly. Phones also let you select how long they should wait before auto-locking themselves. This time should be as short as possible, so that ff you drop your phone or if its is stolen, the timer will expire and the phone will lock before anyone else can access it. 

There are ways to access data on a phone without unlocking it. One way to defend against these methods is for the phone to be fully encrypted. A fully encrypted phone stores all of its data in an unreadable form. When the phone is turned on, nothing can be read until the screen is unlocked the first time, at which point it will decrypt information as needed, and contintue to store information in encyrpted form. This prevents law enforcement from removing the storage chips from your phone or dumping the storage contents to another device, as all of the contents will be unreadable without being unlocked in your specific phone. Luckily, this feature is turned on by default on most phones, so you probably don't have to worry about it! Only phones that are using Android version less than 6 should be concerned. Instructions for those phones are below.
